...
-----------------------------------------------------------------------
-- AmsSp_GetDeviceProperties_2
--
-- Get device properties.
-- This Sp is a updated version of AmsSp_GetDeviceProperties_1
--
-- Inputs -
--	Manufacturer.
--	Protocol.
...
--
-- Outputs -
--	Recordset containing device property / value pair inluding FF Block Info.
--
-- Returns -
--	returns 0 if ok.
...
-- $Revision: 121 $
--
CREATE  PROCEDURE AmsSp_GetDeviceProperties_2
...
declare @SisDevice as nvarchar(2)

Fetch Next from PropsCursor into @sMfr, @sProt, @sMfrId, @sDevTypeCode, @sDevTypeName, @sDevRevCode, @sDevRevName, @sSerial, @sAmsTag, @sAmsDeviceId, @sDisp, @sMajCat, @sMinCat, @sProtRev, @sPlantServerId, @SisDevice
...
begin
    ...
    begin
	select @sColumnName = 'SisDevice'
	select @sColumnValue = @SisDevice
    end
    else
    begin
...
begin
    ...
    where (AmsDeviceId = @sAmsDeviceId) and (BlockIndex = 0)
