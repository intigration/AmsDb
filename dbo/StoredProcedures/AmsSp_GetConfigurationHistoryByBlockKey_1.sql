...
-- $Revision: 121 $
--
CREATE PROCEDURE AmsSp_GetConfigurationHistoryByBlockKey_1
@nBlockKey as integer,
@dtPointInTime as datetime,
...
declare aCursor cursor for select distinct ParamName from BlockData where (BlockKey = @nBlockKey) and (ValueMode = @cValueMode) and (ParamKind = 'P')
...
begin
	...
	WHERE     (dbo.BlockData.ParamKind = 'P') AND
		  ...
		  (dbo.EventLog.EventTime <= @dtPointInTime)
