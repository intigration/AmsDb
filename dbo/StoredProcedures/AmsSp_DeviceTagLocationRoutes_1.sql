...
-----------------------------------------------------------------------
-- AmsSp_DeviceTagLocationRoutes_1
--
-- Get device info along with database location, route location and blockInfoAsXml.
...
-- $Revision: 121 $
--
CREATE PROCEDURE AmsSp_DeviceTagLocationRoutes_1
...
create table #AmsSp_DeviceTagLocationRoutes_1
(
...
declare aCursor cursor for 
	SELECT DISTINCT 
		TOP 100 PERCENT dbo.Manufacturers.Name AS Manufacturer, dbo.MfrProtocols.MfrId, dbo.DeviceProtocols.Name AS Protocol, 
		...
		dbo.RouteFolders.FolderName, dbo.Routes.RouteName, dbo.AmsUdf_GetFFBlockName(dbo.Blocks.BlockType, dbo.Blocks.BlockIndex) AS BlockName, 
		dbo.DeviceLocation.SisStatus AS SisDevice, 
	...
	FROM	dbo.MajorDeviceCategories INNER JOIN
		...
		dbo.DeviceCategories.DeviceCategoryId = dbo.DeviceRevisions.DeviceCategoryId ON 
		dbo.MajorDeviceCategories.MajorDeviceCategoryId = dbo.DeviceCategories.MajorDeviceCategoryId INNER JOIN
		...
		dbo.RouteTags INNER JOIN
		dbo.Routes ON dbo.RouteTags.RouteId = dbo.Routes.RouteId INNER JOIN
		dbo.RouteFolders ON dbo.Routes.FolderId = dbo.RouteFolders.FolderId ON 
		dbo.AmsVw_CurrentTagBlockAsgms.ExtBlockTagKey = dbo.RouteTags.ExtBlockTagKey LEFT OUTER JOIN
...
declare	@FolderName	nvarchar(32)
...
declare	@FinalFolderName	nvarchar(32)
declare	@FinalRouteName	nvarchar(50)
...
--print 'DeviceKey=' + convert(nvarchar(20), @DeviceKey) + ', Prev=' + convert(nvarchar(20), @nPrevDeviceKey)
	if (@DeviceKey = @nPrevDeviceKey and @Protocol = 'FF')
	begin
...
--print 'inserting into #AmsSp_DeviceTagLocationRoutes_1 blockKey- ' + convert(nvarchar(20), @FinalBlockKey)

			insert into #AmsSp_DeviceTagLocationRoutes_1 (Manufacturer,
					MfrId,
					Protocol,
					DeviceTypeCode,
					DeviceTypeName,
		...
		begin
			...
			set @FinalFolderName = @FolderName
			set @FinalRouteName	 = @RouteName
...
EXEC (@sSelectClause  +' from #AmsSp_DeviceTagLocationRoutes_1 ' +  @sWhereClause)

if (@@ERROR <> 0)
	set @iReturnVal = -1

drop table #AmsSp_DeviceTagLocationRoutes_1
